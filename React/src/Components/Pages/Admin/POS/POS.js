...
import './POS.css';
import { BASE_URL } from "./../../../../Components/Apiurl/Api"
...
const POS = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/get-searchproducts`);
  ...
  useEffect(() => {
    const fetchTaxDiscount = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/taxdiscount`);
  ...
  useEffect(() => {
    if (searchQuery.trim()) {
      axios
        .get(`${BASE_URL}/products?search=${searchQuery}`)
  ...
  const handleBarcodeChange = async (e) => {
    ...
    if (inputBarcode.trim()) {
      try {
        const response = await axios.get(`${BASE_URL}/product-by-barcode/${inputBarcode}`);
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      // Save order details
      await axios.post(`${BASE_URL}/save-order`, orderDetails);
