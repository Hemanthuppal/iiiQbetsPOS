...
const Tax = () => {
    ...
    if (sgst && cgst && discount) {
      try {
        if (id) {
          ...
          await axios.put(`${BASE_URL}/taxdiscount/${id}`, { sgst, cgst, discount });
          alert('Tax updated successfully');
          setEditingId(null); // Reset the editing state
        } else {
          // Add new tax data if not editing
          await axios.post(`${BASE_URL}/taxdiscount`, { sgst, cgst, discount });
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`${BASE_URL}/taxdiscount/${id}`);
        fetchTaxData(); // Refresh the list of tax data
      } catch (error) {
