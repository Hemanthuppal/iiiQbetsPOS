...
const ProductList = () => {
  ...
  const handleDeleteProduct = (id) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      ...
      axios
        .delete(`${BASE_URL}/products/${id}`) // Call the backend API
        .then(() => {
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/products`) // Adjust URL to match your API endpoint
      .then((response) => {
  ...
  const columns = [
    ...
    { Header: 'Sale Price', accessor: 'price' },
    {
      ...
      Cell: ({ cell: { value } }) => (
        <img
          src={value.startsWith('/uploads') ? `${BASE_URL}${value}` : value}
          alt="Product"
          style={{ width: '50px' }}
        />
      ),
